...
class WxPayPubHelper extends CController {
    protected static $_wxInstance;
    public static function getWxpayClientPub($className) {
        if (!isset(self::$_wxInstance[$className])) {
            self::$_wxInstance[$className] = new $className;
        }
        return self::$_wxInstance[$className];
    }
}
 ...
 * 所有接口的基类
 */
class Common_util_pub extends CController {
...
class UnifiedOrder_pub extends Wxpay_client_pub {
    ...
    function createXml() {
        try {
            ...
            $AccountID = Yii::app()->session['shopInfo']['AccountID'];
            $params    = OfficialConfigModel::getInstance()->getOneByCond(array('and', 'user_id=' . $AccountID), 'app_id,mch_id');
            if (!$params) {
                echo "getSecret error";
                exit();
            }
            $this->parameters["appid"]            = $params['app_id']; //公众账号ID
            $this->parameters["mch_id"]           = $params['mch_id']; //商户号
...
class JsApi_pub extends Common_util_pub {
    ...
    function createOauthUrlForCode($redirectUrl) {
        $AccountID = Yii::app()->session['shopInfo']['AccountID'];
        $params    = OfficialConfigModel::getInstance()->getOneByCond(array('and', 'user_id=' . $AccountID), 'app_id');
        if (!$params) {
            echo "getSecret error";
            exit();
        }
        $urlObj["appid"]         = $params['app_id'];
    ...
    public function getParameters() {
        $AccountID = Yii::app()->session['shopInfo']['AccountID'];
        $secret    = OfficialConfigModel::getInstance()->getOneByCond(array('and', 'user_id=' . $AccountID), 'app_id,api_key');
        if (!$secret) {
            echo "getSecret error";
            exit();
        }
        $jsApiObj["appId"]     = $secret['app_id'];
        $timeStamp             = time();
        $jsApiObj["timeStamp"] = (string) $timeStamp;
        $jsApiObj["nonceStr"]  = $this->createNoncestr();
        $jsApiObj["package"]   = "prepay_id=" . $this->prepay_id;
        $jsApiObj["signType"]  = "MD5";
        $jsApiObj["paySign"]   = $this->getSign($jsApiObj, $secret['api_key']);
 ...
 * 关闭订单
 */
class CloseOrder extends Wxpay_client_pub {
    ...
    function createXml() {
        try {
            //检测必填参数
            if ($this->parameters["out_trade_no"] == null) {
                throw new SDKRuntimeException("缺少关闭订单接口必填参数out_trade_no！" . "<br>");
            }
            $this->parameters["appid"]     = WxPayConf_pub::APPID; //公众账号ID
            $this->parameters["mch_id"]    = WxPayConf_pub::MCHID; //商户号
            $this->parameters["nonce_str"] = $this->createNoncestr(); //随机字符串
